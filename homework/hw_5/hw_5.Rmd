...
genus<-str_extract_all(data_plant$taxon_name, "^[A-Z][a-z]+")
genus<-matrix(unlist(genus), ncol=1, byrow=TRUE)
#If I add my genus as a list i can cbind but if i unlist it the rows change
...
rand_dpg <- df[sample(nrow(df), size=3), ]
...
taxon_name2<-as.matrix(str_extract_all(data_plant$taxon_name, "^[A-Z][a-z]+\\s"))
data_plant_genus<-cbind(data_plant, genus)
