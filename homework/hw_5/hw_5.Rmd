...
#
data_plant_genus<- data_plant |> mutate( genus= str_extract_all(data_plant$taxon_name, "^[A-Z][a-z]+"))
unlist_genus<-unlist(str_extract_all(data_plant$taxon_name, "^[A-Z][a-z]+"))


#let me know how to get my unlisted values into data_plant table
data_plant_genus<-cbind(data_plant, genus)
colbinded_genus = bind_cols(data_plant,genus)

#use str extract all to match all observations that start with capital alphanumeric followed by any number of lowercase alpha numerics and a space 
#then use cbind to add genus to data plant table
#how do I insert it next to taxon name?
#how do I get it to present like other cols
...
```
when applying unlist to genus column created via mutate
Error: Problem with `mutate()` column `genus`.
ℹ `genus = unlist(str_extract_all(data_plant$taxon_name, "^[A-Z][a-z]+"))`.
ℹ `genus` must be size 915539 or 1, not 915510.
...
taxon_name2<-as.matrix(str_extract_all(data_plant$taxon_name, "^[A-Z][a-z]+\\s[a-z]+"))
data_plant_genus<-cbind(data_plant, taxon_name2)
rand_genus<- data_plant_genus[sample(nrow(data_plant_genus), size=100), ]
```
...
```{r}
#1 M^2 plots make df n_1 siteID vs richness (1M^2)
